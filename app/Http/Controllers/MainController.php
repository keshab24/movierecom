...
class MainController extends Controller
{
    ...
    public function searchMovie(Request $request) {
        ...
        // $movie = Movie::where('title','like','%'.$request->title.'%')->first();
         $searchTerm =$request->title;
       $movie = Movie::where('title', 'LIKE', "%$searchTerm%") // Search movies by title
    // ->orWhereHas('cast', function ($query) use ($searchTerm) {
    //     $query->where('name', 'LIKE', "%$searchTerm%"); // Search movies by cast name
    // })
    ->orWhereHas('getGenres', function ($query) use ($searchTerm) {
        $query->where('genre', 'LIKE', "%$searchTerm%"); // Search movies by genre name
    })
    ->first();
